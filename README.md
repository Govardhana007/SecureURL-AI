...
**🔗 Live demo: [secureurl-ai.onrender.com](https://secureurl-ai.onrender.com)**

> Note: this runs on Render's free tier, which spins the server down after
> periods of inactivity. The first request after a while may take 30–60
> seconds to wake it back up — that's normal, not a bug.
...
This project is already deployed at **[secureurl-ai.onrender.com](https://secureurl-ai.onrender.com)**.

To deploy your own copy (e.g. after forking this repo):
...
Each deploy regenerates the dataset and retrains the model fresh on
Render's servers, so there's nothing to commit or upload beyond the source
files — `dataset/phishing.csv` and `model/phishing_model.pkl` are excluded
via `.gitignore` and rebuilt automatically on every deploy.
